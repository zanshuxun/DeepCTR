...
if __name__ == "__main__":
    ...
    print(len(train_label),sum(train_label))
    input('c')
    ...
    model = DIN(fd, sess_feature, embedding_size=4, att_activation='dice',
                ...
                att_hidden_size=(64, 16,),l2_reg_dnn=1e-5, l2_reg_embedding=1e-5,dnn_dropout=0.2,)
    ...
    # hist_ = model.fit(test_input[:], test_label,
    #                   batch_size=BATCH_SIZE, epochs=1, initial_epoch=0, verbose=1, )

    pred_ans = model.predict(test_input, TEST_BATCH_SIZE)
    print(sum(pred_ans),pred_ans)
